...
// const doubled = nums.map(doubleValues);

function doubleValues(arr1){
    const newArr = arr1.map(function (num) {
        return num * 2;
    })
    return newArr;
}
...
function onlyEvenValues(arr2) {
    const evenArr = arr2.filter(function (num) {
        if (num % 2 === 0) {
...
*/
// const arr3 = ["elie", "colt", "matt", "tim"]

function showFirstAndLast(arr3){
    const firstAndLast = arr3.map(function (name) {
...
function vowelCount(str) {
    ...
    convert.forEach(function (letter) {
...
function doubleValuesWithMap(arr4) {
    const newArr = arr4.map(function (num) {
        return num * 2;
    })
    return newArr;
}
...
function valTimesIndex(arr) {
    const timesArr = arr.map(function (int, i) {
        let final = int * i;
        return final;
    });
    return timesArr;
...
function extractFullName(arr){
    const nameArr = arr.map(function(person) {
...
function filterByValue(arr, key) {
    const catArr = arr.filter(function (person) {
        if (person[key] === true) {
            return person[key];
        }
    })
